...
| 2 | Phase-Coupled Stability Threshold Law | \frac{dG_{ij}}{dt}=\alpha\,\|I_{ij}\|-\mu\,G_{ij}-\lambda\,G_{ij}\,\sin^2\!\left(\frac{\theta_{R,ij}}{2\pi_a}\right) | derived: ARP + Phase-Lift + Adaptive-Ãâ‚¬ | 77 | 28 @ 2026-02-20 | OK | PASS-WITH-ASSUMPTIONS | ./assets/animations/eq-arp-phase-critical-collapse.mp4 | planned | Adds a phase-coupled suppression term to ARP: conductance growth is damped by lifted phase accumulation relative to the local period 2Ãâ‚¬Ã¢â€šÂ. Predicts a sharp stability/transition-like behavior when ÃŽÂ¸_R approaches half-integer multiples of Ãâ‚¬Ã¢â€šÂ (maximal sinÃ‚Â²). Assumptions: ÃŽÂ¸_R,ij is a Phase-Lifted (unwrapped) phase-like observable for edge current; Ãâ‚¬Ã¢â€šÂ sets the relevant phase period; ÃŽÂ» has units 1/time. |
| 3 | ARP Redshift Law (two-timescale extension) | z(t)=z_0\left(1-ae^{-\gamma_1 t}-(1-a)e^{-\gamma_2 t}\right),\quad 0\le a\le1 | derived mapping extension (model selection) | 73 | 22 @ 2026-02-22 | OK | PASS-WITH-ASSUMPTIONS | planned | planned | Two-timescale ARP redshift relaxation captures fast+slow approach to asymptote and supports AIC/BIC model selection against the single-timescale baseline. |
| 4 | Phase-Lifted Complex Conductance Update | \frac{d\tilde G_{ij}}{dt}=\alpha_G\,\|I_{ij}(t)\|\,e^{i\theta_{R,ij}(t)}-\mu_G\,\tilde G_{ij}(t),\qquad \theta_{R,ij}(t)=\mathrm{unwrap}\!\big(\arg I_{ij}(t);\theta_{\rm ref},\pi_a\big) | derived: ARP core + Phase-Lift + Adaptive-Ãâ‚¬ | 71 | 26 @ 2026-02-20 | OK | PASS-WITH-ASSUMPTIONS | ./assets/animations/eq-arp-phase-lifted-complex-conductance.mp4 | planned | Complex-admittance lift of ARP: conductance grows along the instantaneous current phasor direction using a Phase-Lifted (unwrapped) phase. Assumes phase-coherent transport where a complex ~G is meaningful. Optional variant: include a Z2 parity factor b_ij = (-1)^{w_ij} multiplying e^{iÃŽÂ¸_R,ij} to model sign flips under branch crossings. |
...
| 2026-02 | Phase-Coupled Stability Threshold Law | \frac{dG_{ij}}{dt}=\alpha\,\|I_{ij}\|-\mu\,G_{ij}-\lambda\,G_{ij}\,\sin^2\!\left(\frac{\theta_{R,ij}}{2\pi_a}\right) | derived: ARP + Phase-Lift + Adaptive-Ãâ‚¬ | PASS-WITH-ASSUMPTIONS | 77 | ./assets/animations/eq-arp-phase-critical-collapse.mp4 | planned |
| 2026-02 | Phase-Lifted Complex Conductance Update | \frac{d\tilde G_{ij}}{dt}=\alpha_G\,\|I_{ij}(t)\|\,e^{i\theta_{R,ij}(t)}-\mu_G\,\tilde G_{ij}(t),\qquad \theta_{R,ij}(t)=\mathrm{unwrap}\!\big(\arg I_{ij}(t);\theta_{\rm ref},\pi_a\big) | derived: ARP core + Phase-Lift + Adaptive-Ãâ‚¬ | PASS-WITH-ASSUMPTIONS | 71 | ./assets/animations/eq-arp-phase-lifted-complex-conductance.mp4 | planned |
