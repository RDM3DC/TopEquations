...
class AutoEquation(Scene):
    def construct(self):
        title = Text('Phase-Lifted Complex Conductance Update', font_size=52)
        title.to_edge(UP)

        eq = MathTex('\\frac{d\\tilde G_{ij}}{dt}=\\alpha_G\\,|I_{ij}(t)|\\,e^{i\\theta_{R,ij}(t)}-\\mu_G\\,\\tilde G_{ij}(t),\\qquad \\theta_{R,ij}(t)=\\mathrm{unwrap}\\!\\big(\\arg I_{ij}(t);\\theta_{\\rm ref},\\pi_a\\big)').scale(1.25)
        eq.next_to(title, DOWN, buff=0.55)

        what = Text('Complex-admittance lift of ARP: conductance grows along the instantaneous current phasor direction using a Phase-Lifted…', font_size=30, color=BLUE_A)
