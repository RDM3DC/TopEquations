...
class AutoEquation(Scene):
    def construct(self):
        title = Text('Shielding Mechanism Law (ARP)', font_size=52)
        title.to_edge(UP)

        eq = MathTex('E_{eff}=(1-\\lambda)E_{ext},\\ \\ J_{adapt}=\\sigma E_{ext},\\ \\ 0<\\lambda<1').scale(1.25)
        eq.next_to(title, DOWN, buff=0.55)

        what = Text('Proposes a shielding effect in ARP systems, modeled as a fractional reduction of the external field. The effective fiel…', font_size=30, color=BLUE_A)
