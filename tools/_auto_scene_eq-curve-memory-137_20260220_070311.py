...
class AutoEquation(Scene):
    def construct(self):
        title = Text('Curve Memory Fine-Structure Emergence', font_size=52)
        title.to_edge(UP)

        eq = MathTex('\\alpha^{-1}\\approx 137\\ \\ \\text{(emergent resonance condition in curve memory)}').scale(1.25)
        eq.next_to(title, DOWN, buff=0.55)

        what = Text('Formalizes the emergence of the inverse fine-structure constant (α⁻¹ ≈ 137) via resonant harmonic conditions in curve m…', font_size=30, color=BLUE_A)
