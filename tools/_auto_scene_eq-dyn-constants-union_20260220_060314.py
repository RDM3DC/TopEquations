...
class AutoEquation(Scene):
    def construct(self):
        title = Text('Unified Dynamic Constants Law', font_size=52)
        title.to_edge(UP)

        eq = MathTex('\\dot{\\mathbf{c}}=A\\,\\mathbf{c}\\ \\ \\text{with}\\ \\mathbf{c}=[\\pi_a, e, \\varphi, c]^T').scale(1.25)
        eq.next_to(title, DOWN, buff=0.55)

        what = Text('First-order linear coupling for adaptive updates of πₐ, e, ϕ, and c, expressing networked rates of change—models the un…', font_size=30, color=BLUE_A)
